...
class BwcTest(unittest.TestCase):
    ...
    def test_upgrade_from_057_to_latest(self):
        """ Test upgrade path from 0.57 to latest

        Creates a blob and regular table in 0.57 and inserts a record, then
        goes through all VERSIONS - each time verifying that a few simple
        selects work.
        """
        print(f'# Starting: 0.57.x')
        node = self._new_node('0.57.x')
        node.start()
